  ...
  return (
    <div id="skills" className="bg-[#23263a] h-full flex flex-col items-center">
      <div className="mb-10 mt-10 tablet:mt-16 flex flex-col w-full p-5 animation-title opacity-0 relative bottom-4">
        <p className="text-white font-lucio font-semibold tablet:text-xl notebook:text-2xl">{changeLanguage ? 'Services' : 'Servicios'}</p>
      ...
      <div className="w-full flex justify-center gap-[50px] flex-wrap">
        {getResponse.map((item) => (
          <div key={item._id}>
            ...
            <div className={`window-animation-${item.title.replace(/\s/g, "")} relative -left-[1000px] notebook:-left-[2000px] flex justify-center`}>
              <SkillWindow
                title={item.title}
