const AboutMeSpanish = () => {
  return (
    <div>
      <p className="text-white font-paragraph text-sm p-5 tablet:text-base notebook:text-lg">
        Hola!, Mi nombre es Lucio y soy Desarrollador Full Stack.
        <span className="block mt-2"></span>
        Soy un apasionado de la tecnología desde que tengo uso de razón, por eso
        decidí meterme en este hermoso mundo.
        <span className="block mt-2"></span>
        Actualmente estoy más enfocado en el lado del servidor (backend) ya que
        es en lo que más me destaco, también estoy bastante familizarizado con
        la parte del frontend, por lo cual puedo hacer las dos cosas sin
        problemas.
      </p>
      <p className="text-white font-paragraph text-sm p-5 tablet:text-base notebook:text-lg">
        Podrás encontrar mis proyectos, con su documentación necesaria y
        deployados aquí:
      </p>
    </div>
  );
};

export default AboutMeSpanish;
